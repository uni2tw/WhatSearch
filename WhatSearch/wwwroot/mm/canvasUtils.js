...
canvasItems.getDestCanvasSize = function(mergeType, mergeDirection, maxWidth) {
    ...
    for (var i = 0; i < this.length; i++) {
        ...
        if (mergeDirection == 1) { // 水平合併
            result.width += adjustedWidth;
            result.height = Math.max(result.height, adjustedHeight);
        } else { // 垂直合併
...
//maxWidth 會在mobile啟用，原因是因為記憶體可能不足，造成合併失敗
canvasItems.createDestCanvas = function(mergeType, mergeDirection, maxWidth) {
    var destCanvasWidth = 0;
    var destCanvasHeight = 0;
    
    var destCanvasSize = this.getDestCanvasSize(mergeType, mergeDirection, maxWidth);
    ...
    for (var i = 0; i < this.length; i++) {
        ...
        if (mergeDirection == 0) { // 垂直合併
            destCtx.drawImage(img, 0, 0, img.width, img.height, 
                0, position, adjustedSize.width, adjustedSize.height);
            position += adjustedSize.height;
        } else if (mergeDirection == 1) { // 水平合併
            destCtx.drawImage(img, 0, 0, img.width, img.height, 
                position, 0, adjustedSize.width, adjustedSize.height);
            position += adjustedSize.width;
        }
    }
...
window.downloadImage = function() {
    ...
    let mergeDirection = parseInt($('#mergeDirection').val(), 10);

    var destCanvas = canvasItems.createDestCanvas(mergeType, mergeDirection);
